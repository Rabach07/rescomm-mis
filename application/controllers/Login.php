...
class Login extends CI_Controller {
	...
	public function index() {
		if(!$this->user_model->is_logged_in()){
            if($this->session->flashdata('p3m_pesan_error')) {
                $data['error'] = $this->session->flashdata('p3m_pesan_error');
                //$this->session->unset_userdata('p3m_pesan_error');
                $this->load->view('Backend/login_view',$data);
            } else if($this->session->flashdata('p3m_pesan_sukses')) {
                $data['sukses'] = $this->session->flashdata('p3m_pesan_sukses');
	...
	public function dologin() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if(!$flag) {
            	$this->session->set_flashdata('p3m_pesan_error', $this->user_model->error_messages());
            	redirect('login');
            } else {
                $urlke = $this->session->userdata('p3m_urlke') == NULL ? 'dashboard' : $this->session->userdata('p3m_urlke');
                $this->session->unset_userdata('p3m_urlke');
        	...
        	$this->session->set_flashdata('p3m_pesan_error', validation_errors());
            redirect('login');
    ...
    public function dolupa() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if(!$flag) {
            	...
            	$this->session->set_flashdata('p3m_pesan_sukses', "Link Ganti Password telah dikirim ke email Anda");
            	redirect('login/lupa');
            }
            //redirect('daftar');
